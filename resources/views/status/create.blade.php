...
@section('container')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Create Status</h1>
    </div>

    <div class="col-lg-8">

        <form method="post" action="/status" class="mb-5">
            @csrf
            <div class="mb-3">
                <label for="nama_status" class="form-label">Name</label>
                <input type="text"
                    class="form-control @error('nama_status')
                    is-invalid
                @enderror"
                    id="nama_status" name="nama_status" required autofocus value="{{ old('nama_status') }}">
                @error('nama_status')
